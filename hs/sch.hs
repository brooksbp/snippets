...
-- (eqv? 2 "2")   = #f
-- (equal? 2 "2") = #t
-- obvious solution is to mapM over list of unpacker functions, but
-- cannot do that since they return different types, so use a
-- heterogenous list:

data Unpacker = forall a. Eq a => AnyUnpacker (LispVal -> ThrowsError a)
-- "for any type that is an instance of Eq, you can define an Unpacker
